 ...
 * @author Chris
 */
public class OpenHillShadeListener_June implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_June(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("June working");
  
  // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open June hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("June hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_June[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_June.length; i++){
        for(int j = 0; j<hillshade_June[i].length; j++){
           
           if ((hillshade_June[i][j] != -9999)&& (hillshade_June[i][j] != 0)){
           double percentage1 = (hillshade_June[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_June[i][j] = percentage2;
           }
           
           else if (hillshade_June[i][j] == 0){
           hillshade_June[i][j] = 0;
           }
           
           else {
           hillshade_June[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_June(hillshade_June);
    ...
    System.out.println("June hillshade uploaded");
