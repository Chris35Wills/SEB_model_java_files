 ...
 * ****************************************************************************
 * 
 * A listener which prints out a 2D array in command line of the elevation surface
 * opened via the OpenElevationListener, using methods declared in Storage.java
 * @author Chris Williams 2/11/10
 **/
public class PrintElevationArrayListener implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  PrintElevationArrayListener(GUIPanel pn, Storage s)
  {
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    try
    {
      ...
      elevationArray = store.getElevation();


      for (int i = 0; i < store.getElevation().length; i++)
      {
        for (int j = 0; j < store.getElevation()[i].length; j++)
        {
          System.out.print(store.getElevation()[i][j] + " ");
        }
        System.out.print(" ");
      }
