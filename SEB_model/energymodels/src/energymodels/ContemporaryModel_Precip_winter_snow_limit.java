 ...
 * 
 * NB/ Regarding the on-going issue of there being too much winter snow lasting 
 * through when density is set to 400kg m^3, this is fixed when desnity is reduced.
 * The density itself shouldn't be changed (the density is the density after all)
 * so a factor will be introduced to strip the snow - this factor will be 
 * multiplied by the density. So, instead of reducing density to 200kg m^3, we 
 * will multiply density by a factor of 0.5. This factor is then tested in the 
 * sensitivity analysis and prevents medlling with known density values.
 * 
 ...
 * @author Chris
 */
public class ContemporaryModel_Precip_winter_snow_limit implements ActionListener {

    private GUIPanel panel = null;
    private Storage store = null;

    ContemporaryModel_Precip_winter_snow_limit(GUIPanel panel, Storage store) {
...
//
                 ...
                 */
                if (save_arrays == 3) {
                    File f_glacier_slope = new File("F:/Melt_modelling/Model_outputs/slope_" + (int) day + "." + (int) month + "." + (int) year + ".txt");
                 ...
                 */
                if (save_arrays == 3) {
                    File f_aspectSurface = new File("F:/Melt_modelling/Model_outputs/aspect_" + (int) day + "." + (int) month + "." + (int) year + ".txt");
