 ...
 * @author Chris
 */
public class OpenHillShadeListener_Oct implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_Oct(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("October working");
  
    // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open October hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("October hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_Oct[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_Oct.length; i++){
        for(int j = 0; j<hillshade_Oct[i].length; j++){
           
           if ((hillshade_Oct[i][j] != -9999)&& (hillshade_Oct[i][j] != 0)){
           double percentage1 = (hillshade_Oct[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_Oct[i][j] = percentage2;
           }
           
           else if (hillshade_Oct[i][j] == 0){
           hillshade_Oct[i][j] = 0;
           }
           
           else {
           hillshade_Oct[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_Oct(hillshade_Oct);
    ...
    System.out.println("October hillshade uploaded");
