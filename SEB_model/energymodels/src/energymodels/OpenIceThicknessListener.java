 ...
 * ****************************************************************************
 * 
 * This class creates an "Open a File" dialog box when the user clicks on the "Open Ice Thickness Surface"
 ...
 **/
public class OpenIceThicknessListener implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenIceThicknessListener(GUIPanel pn, Storage s)
  {
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    FileDialog fd = new FileDialog(new Frame (), "Open an ice thickness surface file", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("Glacier thickness file not uploaded");
    return;
    }
    
    else{
    //File file = new File("F:\\Model_Test_Files\\59posthickness_5m.txt");

    // Creates a GRIDJava object from the opened file

    GRIDJava2 gj = new GRIDJava2(file, true, store);

    // Adds the GRIDJava2 object to the panel

    panel.addGRIDJavaMethod(gj);

    // Creates a 2D array from the GRIDJava object

    double thick_data[][] = gj.getTwoDdoubleArray();
    ...
    int thick_data1d[] = gj.getOneDintArray();
    ...
    MemoryImageSource mis = new MemoryImageSource(gj.getNumberOfColumns(), gj.getNumberOfRows(), thick_data1d, 0, gj.getNumberOfColumns());
    ...
    //double[][] iceThicknessSize = store.getThickness();

    //System.out.println("iceThicknessSize.length = " + iceThicknessSize.length + " iceThicknessSize[0].length = " + iceThicknessSize[0].length);

    System.out.println("Ice Thickness Surface uploaded");
        }
  }
}
