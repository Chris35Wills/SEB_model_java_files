 ...
 * @author Chris
 */
public class OpenHillShadeListener_Nov implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_Nov(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("November working");
   // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open November hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("November hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_Nov[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_Nov.length; i++){
        for(int j = 0; j<hillshade_Nov[i].length; j++){
           
           if ((hillshade_Nov[i][j] != -9999)&& (hillshade_Nov[i][j] != 0)){
           double percentage1 = (hillshade_Nov[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_Nov[i][j] = percentage2;
           }
           
           else if (hillshade_Nov[i][j] == 0){
           hillshade_Nov[i][j] = 0;
           }
           
           else {
           hillshade_Nov[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_Nov(hillshade_Nov);
    ...
    System.out.println("November hillshade uploaded");
