 ...
 * @author Chris
 */
public class OpenHillShadeListener_Jan implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_Jan(GUIPanel pn, Storage s)
  {
  ...
  public void actionPerformed(ActionEvent e)
  {
      ...
      System.out.println("January working");
  
   // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open January hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("January hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_Jan[][] = gj.getTwoDdoubleArray();

    //store.setHillshade(data);
    
    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_Jan.length; i++){
        for(int j = 0; j<hillshade_Jan[i].length; j++){
           
           if ((hillshade_Jan[i][j] != -9999)&& (hillshade_Jan[i][j] != 0)){
           double percentage1 = (hillshade_Jan[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_Jan[i][j] = percentage2;
           }
           
           else if (hillshade_Jan[i][j] == 0){
           hillshade_Jan[i][j] = 0;
           }
           
           else {
           hillshade_Jan[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_Jan(hillshade_Jan);
    ...
    System.out.println("January hillshade uploaded");
