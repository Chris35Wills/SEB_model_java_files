 ...
 * @author Chris
 */
public class OpenHillShadeListener_July implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_July(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("July working");
  
    // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open July hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("July hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_July[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_July.length; i++){
        for(int j = 0; j<hillshade_July[i].length; j++){
           
           if ((hillshade_July[i][j] != -9999)&& (hillshade_July[i][j] != 0)){
           double percentage1 = (hillshade_July[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_July[i][j] = percentage2;
           }
           
           else if (hillshade_July[i][j] == 0){
           hillshade_July[i][j] = 0;
           }
           
           else {
           hillshade_July[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_July(hillshade_July);
    ...
    System.out.println("July hillshade uploaded");
