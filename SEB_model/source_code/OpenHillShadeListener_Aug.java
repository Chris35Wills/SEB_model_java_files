 ...
 * @author Chris
 */
public class OpenHillShadeListener_Aug implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_Aug(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("August working");
  
    // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open August hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("August hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_Aug[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_Aug.length; i++){
        for(int j = 0; j<hillshade_Aug[i].length; j++){
           
           if ((hillshade_Aug[i][j] != -9999)&& (hillshade_Aug[i][j] != 0)){
           double percentage1 = (hillshade_Aug[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_Aug[i][j] = percentage2;
           }
           
           else if (hillshade_Aug[i][j] == 0){
           hillshade_Aug[i][j] = 0;
           }
           
           else {
           hillshade_Aug[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_August(hillshade_Aug);
    ...
    System.out.println("August hillshade uploaded");
