 ...
 * @author Chris
 */
public class OpenHillShadeListener_Sept implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_Sept(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("September working");
  
  // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open September hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("September hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_Sept[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_Sept.length; i++){
        for(int j = 0; j<hillshade_Sept[i].length; j++){
           
           if ((hillshade_Sept[i][j] != -9999)&& (hillshade_Sept[i][j] != 0)){
           double percentage1 = (hillshade_Sept[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_Sept[i][j] = percentage2;
           }
           
           else if (hillshade_Sept[i][j] == 0){
           hillshade_Sept[i][j] = 0;
           }
           
           else {
           hillshade_Sept[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_Sept(hillshade_Sept);
    ...
    System.out.println("September hillshade uploaded");
