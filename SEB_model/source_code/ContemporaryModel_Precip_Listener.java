...
* 
* STILL NEED TO INTEGRATE SUMMER AND WINTER SNOW LAYERS INTO MELT - REMEMBER 
* DIFF. DENSITIES AND THAT ANY REMAINING ENERGY SHOULD THEN BE USED TO MELT ICE
* 
* Line details:
* 
 ...
 * @author Chris
 */
public class ContemporaryModel_Precip_Listener implements ActionListener {
   
  private GUIPanel panel = null;
  private Storage store = null;
  
  ContemporaryModel_Precip_Listener (GUIPanel panel, Storage store)
  {
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    double summer_snow_density = 400.0; // Check and change this value
    double winter_snow_density = 400.0; // Check and change this value
    double ice_mass;
    double snow_mass;
    ...
    //**************Summer precipitation************** 
      ...
      System.out.println("Winter precipitation");    
           ...
           * 
           * IMPORTANT: This needs to prevent winter snow depth from exceeding 
           * maximum snow depth at given cell position at the end of a given 
           * season.
           */
...
//    double Q_counter = 0;
       ...
       for(int i = 0; i<elevation.length; i++){
		...
		if((summer_snow_thickness[i][j] != NODATA_value)&&(summer_snow_thickness[i][j] >= threshold_snow_thickness_for_albedo))
		{
		albedo_instance = snow_albedo;
               // System.out.println("Albedo set to that of snow (summer)");
		}
		else if((summer_snow_thickness[i][j] != NODATA_value)&&(summer_snow_thickness[i][j] < threshold_snow_thickness_for_albedo)&&(winter_snow_thickness[i][j] != NODATA_value)&&(winter_snow_thickness[i][j] >= threshold_snow_thickness_for_albedo))
		{
		albedo_instance = snow_albedo;
     ...
     /**
       * Surface melting.
       * 
       * Melts through the summer snow surface, then the winter surface and 
       * then the ice surface. Accounts for remaining Q, where Q available > 
       * Q required to melt. In this way, energy is conserved and used on the 
       * next layer that can be melted.
       **/  
...
for(int i = 0; i<summer_snow_thickness.length; i++){
		...
		if((summer_snow_surface_instance != NODATA_value) && (Q_for_melt != NODATA_value) && (summer_snow_surface_instance > 0) && (Q_for_melt > 0)){
	    ...
	    double summer_snow_volume = summer_snow_surface_instance * cell_area;
			...
			// Update summer_snow_surface_instance (if any cell = -9999, then it remains -9999; if any reduction is greater than cell thickness, cell is turned to 0)
			
			if(((summer_snow_surface_instance - summer_snow_surface_change_instance)<0) && (summer_snow_surface_instance != NODATA_value))
			{
				summer_snow_surface_instance = 0;
			}
			else if(((summer_snow_surface_instance - summer_snow_surface_change_instance)>=0) && (summer_snow_surface_instance != NODATA_value))
			{
				summer_snow_surface_instance = summer_snow_surface_instance - summer_snow_surface_change_instance;
			}
			else
			{
				summer_snow_surface_instance = NODATA_value;
			}
			...
			if((energy_for_total_summer_snow_melt - Q_for_melt)<0)			 // A negative value is indicative of Q_for_melt > Q required so gives you the remainding energy
			{
				 Q_for_melt = (energy_for_total_summer_snow_melt - Q_for_melt)*-1; // Inverts the negative value to make the remainding energy positive
			}
			else
			{
				 Q_for_melt = 0.0;
		...
		// Now deal with WINTER snow
			...
			if((winter_snow_surface_instance != NODATA_value) && (Q_for_melt != NODATA_value) && (winter_snow_surface_instance > 0) && (Q_for_melt > 0)){
		
			
            double winter_snow_volume = winter_snow_surface_instance * cell_area;
            double winter_snow_mass = winter_snow_volume * winter_snow_density;
			...
			// Update winter_snow_surface_instance (if any cell = -9999, then it remains -9999; if any reduction is greater than cell thickness, cell is turned to 0)
			
			if(((winter_snow_surface_instance - winter_snow_surface_change_instance)<0) && (winter_snow_surface_instance != NODATA_value))
			{
				winter_snow_surface_instance = 0;
			}
			else if(((winter_snow_surface_instance - winter_snow_surface_change_instance)>=0) && (winter_snow_surface_instance != NODATA_value))
			{
				winter_snow_surface_change_instance = winter_snow_surface_instance - winter_snow_surface_change_instance;
			}
			else
			{
				winter_snow_surface_instance = NODATA_value;
			}
			
			// Calculates the remaining energy available (i.e. used in instances where there is more energy available than there is snow to melt)
			
			if((energy_for_total_winter_snow_melt - Q_for_melt)<0)			 // A negative value is indicative of Q_for_melt > Q required so gives you the remainding energy
			{
				 Q_for_melt = (energy_for_total_winter_snow_melt - Q_for_melt)*-1; // Inverts the negative value to make the remainding energy positive
			}
			else
			{
				 Q_for_melt = 0.0;
	    ...
	    // If snow thickness is equal to 0 or there is no energy
            ...
            else if ((winter_snow_surface_instance == 0) | (Q_for_melt <= 0)) // Required so that if winter_snow_surface is already zero or there is no energy, then nothing is done
            {
             winter_snow_surface_change_instance = 0;
             winter_snow_surface_change[i][j] = 0;
            }
	   ...
	   if((glacier_thickness_instance != NODATA_value) && (Q_for_melt != NODATA_value) && (glacier_thickness_instance > 0) && (Q_for_melt > 0)){
	    ...
	    //double thickness = glacier_thickness_instance;
			...
			// Update ice_thickness (if any cell = -9999, then it remains -9999; if any reduction is greater than cell thickness, cell is turned to 0)
			
			if(((glacier_thickness_instance - ice_thickness_change)<0) && (glacier_thickness_instance != NODATA_value))
			{
				glacier_thickness_instance = 0;
			}
			else if(((glacier_thickness_instance - ice_thickness_change)>=0) && (glacier_thickness_instance != NODATA_value))
			{
				glacier_thickness_instance = glacier_thickness_instance - ice_thickness_change;
			}
			else
			{
				glacier_thickness_instance = NODATA_value;
			}
		...
		else if((summer_snow_surface_instance != NODATA_value) && (Q_for_melt != NODATA_value) && (summer_snow_surface_instance == 0) && (Q_for_melt > 0)){
	   ...
	   if((glacier_thickness_instance != NODATA_value) && (Q_for_melt != NODATA_value) && (glacier_thickness_instance > 0) && (Q_for_melt > 0)){
		...
		//Arrays = instances...
		summer_snow_thickness[i][j] = summer_snow_surface_instance;
		winter_snow_thickness[i][j] = winter_snow_surface_instance;
		glacier_thickness[i][j] = glacier_thickness_instance;
...
/**
* This bit will open a save box to save the surface radiation surface 
...
///**
//* This bit will open a save box to save the Psi surface layer as an 
...
//    
    ...
    File f_psi_surface = new File("F:/Melt_modelling/Model_outputs/psi_" + (int)day + "." + (int)month + "." + (int)year + ".txt"); 
...
/**
* This bit will open a save box to save the Q daily grid layer as an 
* ASCII, using the coordinates of the originally opened up elevation surface
**/    

    File f_q_daily = new File("F:/Melt_modelling/Model_outputs/Q_daily_grid_" + (int)day + "." + (int)month + "." + (int)year + ".txt"); 
...
/**
* This bit will open a save box to save the updated thickness layer as an 
* ASCII, using the coordinates of the originally opened up elevation surface
**/    

    File f_glacier_thickness = new File("F:/Melt_modelling/Model_outputs/updated_thickness_" + (int)day + "." + (int)month + "." + (int)year + ".txt");
...
**/    
    ...
    File f_elevation_update = new File("F:/Melt_modelling/Model_outputs/updated_elevation_" + (int)day + "." + (int)month + "." + (int)year + ".txt");
