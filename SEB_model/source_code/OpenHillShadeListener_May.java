 ...
 * @author Chris
 */
public class OpenHillShadeListener_May implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_May(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("May working");
  
    // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open May hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("May hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_May[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_May.length; i++){
        for(int j = 0; j<hillshade_May[i].length; j++){
           
           if ((hillshade_May[i][j] != -9999)&& (hillshade_May[i][j] != 0)){
           double percentage1 = (hillshade_May[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_May[i][j] = percentage2;
           }
           
           else if (hillshade_May[i][j] == 0){
           hillshade_May[i][j] = 0;
           }
           
           else {
           hillshade_May[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_May(hillshade_May);
    ...
    System.out.println("May hillshade uploaded");
