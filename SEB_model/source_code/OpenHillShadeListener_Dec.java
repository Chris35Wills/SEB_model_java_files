 ...
 * @author Chris
 */
public class OpenHillShadeListener_Dec implements ActionListener
{

  private GUIPanel panel = null;
  private Storage store = null;

  OpenHillShadeListener_Dec(GUIPanel pn, Storage s)
  {
  ...
  {
  System.out.println("December working");
   // Creates the "Open a File" dialog box

    FileDialog fd = new FileDialog(new Frame (), "Open December hillshade ASCII", FileDialog.LOAD);
    fd.setVisible(true);
    ...
    {
    System.out.println("December hillshade file not uploaded");
    return;
    }
    ...
    double hillshade_Dec[][] = gj.getTwoDdoubleArray();

    // Converts hillshade values (0-255) to %
          
       for(int i = 0; i<hillshade_Dec.length; i++){
        for(int j = 0; j<hillshade_Dec[i].length; j++){
           
           if ((hillshade_Dec[i][j] != -9999)&& (hillshade_Dec[i][j] != 0)){
           double percentage1 = (hillshade_Dec[i][j]/255.0); //half the way.... (* 100)
           double percentage2 = percentage1 * 100.0;
           hillshade_Dec[i][j] = percentage2;
           }
           
           else if (hillshade_Dec[i][j] == 0){
           hillshade_Dec[i][j] = 0;
           }
           
           else {
           hillshade_Dec[i][j] = -9999;          
           }
           
        }
       }
           
    store.setHillshade_Dec(hillshade_Dec);
    ...
    System.out.println("December hillshade uploaded");
